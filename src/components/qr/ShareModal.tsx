...
const SITE_URL = 'https://free-qr-code.codama.dev/'
const DISMISS_KEY = 'shareModalDismissed'
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="overflow-hidden p-0 sm:max-w-lg">
        ...
        <div className="relative bg-linear-to-br from-orange-500 via-amber-400 to-orange-500 px-6 pt-10 pb-6 text-center text-white">
          ...
          <div className="relative mx-auto mt-5 inline-block rounded-2xl bg-white p-3 shadow-orange-900/20 shadow-xl">
            <StyledQRCode
              value={SITE_URL}
              size={160}
              level="H"
              ...
              includeMargin
              role="img"
              aria-label="QR code for free-qr-code.codama.dev"
            />
          </div>
          <p className="relative mt-3 font-medium text-white/80 text-xs">
            {t('share.scanOrShare')}
          </p>
        </div>
        ...
        <div className="px-6 pt-4 pb-2">
          ...
          <div className="flex justify-center gap-3">
            {SHARE_TARGETS.map(target => (
              <a
                key={target.key}
                href={target.getUrl(shareText, SITE_URL)}
                target="_blank"
